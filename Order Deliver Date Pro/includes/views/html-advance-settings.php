...
<div class="orddd_weekday_settings_heading"> <?php _e( 'Use the below table if you want to create different settings for each weekday. You can fill in the required settings & leave the rest as blank if not needed. For example, if you only want delivery charges per weekday, but not Minimum Time or Same-day delivery, then you can enter the charges & leave other fields blank. If multiple settings are added for one Weekday, then settings which are added last will be considered.<a href="https://www.tychesoftwares.com/docs/docs/order-delivery-date-pro-for-woocommerce/weekday-settings/?utm_source=userwebsite&utm_medium=link&utm_campaign=OrderDeliveryDateProSetting" target="_blank" class="dashicons dashicons-external" style="line-height:unset;"></a>', 'order-delivery-date' ); ?></div>
<br>
<table class="orddd-advance-settings-table widefat">
    <thead>
        <tr>
            <th><?php _e( 'Weekday', 'order-delivery-date' ); ?>&nbsp;<abbr class=\"required\" title=\"required\" style="color: red;">*</abbr>&nbsp;<?php echo $weekday_help_tip ?></th>
            <th><?php _e( 'Charges', 'order-delivery-date' ); ?>&nbsp;<?php echo $charges_help_tip; ?></th>
            <th><?php _e( 'Checkout label', 'order-delivery-date' ); ?>&nbsp;<?php echo $charges_label_help_tip; ?></th>
            <th><?php _e( 'Same day cut-off', 'order-delivery-date' ); ?>&nbsp;<?php echo $same_day_cut_off_help_tip; ?></th>
            <th><?php _e( 'Next day cut-off', 'order-delivery-date' ); ?>&nbsp;<?php echo $next_day_cut_off_help_tip ?></th>
            <th><?php _e( 'Weekday before next day cut-off time', 'order-delivery-date' ); ?>&nbsp;<?php echo $weekday_before_help_tip; ?></th>
            <th><?php _e( 'Weekday after next day cut-off time', 'order-delivery-date' ); ?>&nbsp;<?php echo $weekday_after_help_tip ?></th>
            <th><?php _e( 'Minimum Time', 'order-delivery-date' ); ?><?php echo $minimum_time_help_tip; ?></th>
        </tr>
    </thead>
    <tfoot>
        <tr>
            <th colspan="8"> 
                <a class="button plus orddd_advance_settings_insert"><?php _e( 'Insert row', 'order-delivery-date' ); ?></a>
                <a class="button minue orddd_advance_settings_remove"><?php _e( 'Remove selected row' , 'order-delivery-date' ); ?></a>
            </th> 
        </tr>
    </tfoot>
    <tbody id="settings">
...
<br>
<div><?php _e( 'If the cut-off time for Same day / Next day delivery is same for all the weekdays then <a href="admin.php?page=order_delivery_date&action=general_settings&section=time_settings">click here</a> to make the necessary changes.' ); ?> </div>
    ...
    <tr class="orddd_advance_setting_row" data-tip="<?php echo esc_attr( sprintf( __( 'Advance Setting Row: %s', 'order-delivery-date' ), '{{ data.row_id }}' ) ); ?>" data-id="{{ data.row_id }}">
        <td class="weekdays">
            <select class="orddd_weekdays" id="orddd_weekdays" name="orddd_weekdays[{{ data.row_id }}]" placeholder="Select Weekdays" data-attribute="orddd_weekdays">
                <option name="select_weekday" value=""><?php _e( 'Weekday', 'order-delivery-date' ); ?></option>
                <?php foreach ( $orddd_weekdays as $n => $day_name ) {
                    ?>
                    <option name="<?php echo $n; ?>" value="<?php echo $n; ?>" <# if ( data.orddd_weekdays == "<?php echo $n; ?>" ) { #> selected <# } #> > <?php echo $day_name; ?> </option> <?php
                } ?>
            </select>
        </td>
        <td class="additional_charges">
            <input type="text" value="{{ data.additional_charges }}" name="additional_charges[{{ data.row_id }}]" class="orddd_additional_charges" data-attribute="additional_charges" placeholder="In <?php echo get_woocommerce_currency_symbol(); ?>" />
        </td>
        ...
        <td class="disable_same_day_delivery_after_hours">
            ...
            if( 'on' == get_option( 'orddd_enable_same_day_delivery' ) ) {
                ...
                ?>
                <option name="time" value=""> <?php _e( 'Time', 'order-delivery-date' ); ?></option> 
                <?php
                $from_start_ts   = strtotime( "00:00" );
                $to_end_ts       = strtotime( "23:59" );
                while( $from_start_ts <= $to_end_ts ) {
                    $time_to_display = date( 'H:i', $from_start_ts );
                    
                    ?>
                    <option name="<?php echo $time_to_display; ?>" value="<?php echo $time_to_display; ?>" <# if ( data.orddd_disable_same_day_delivery_after_hours == "<?php echo $time_to_display; ?>" ) { #> selected <# } #> > <?php echo $time_to_display; ?> </option> 
                    <?php
        ...
        <td class="disable_next_day_delivery_after_hours">
            ...
            if( 'on' == get_option( 'orddd_enable_next_day_delivery' ) ) {
                ...
                while( $from_start_ts <= $to_end_ts ) {
                    ...
                    ?>
                    <option name="<?php echo $time_to_display;?>" value="<?php echo $time_to_display; ?>" <# if ( data.orddd_disable_next_day_delivery_after_hours == "<?php echo $time_to_display; ?>" ) { #> selected <# } #> > <?php echo $time_to_display; ?> </option>
                    <?php
        ...
        <td class="before_cutoff_weekday">
            ...
            if( 'on' == get_option( 'orddd_enable_next_day_delivery' ) ) {
                ...
                <?php foreach ( $orddd_weekdays as $n => $day_name ) {
                    ?>
                    <option name="<?php echo $n; ?>" value="<?php echo $n; ?>" <# if ( data.orddd_before_cutoff_weekday == "<?php echo $n; ?>" ) { #> selected <# } #> > <?php echo $day_name; ?> </option> <?php
                } ?>
            </select>
        </td>   
        ...
        <td class="after_cutoff_weekday">
            ...
            if( 'on' == get_option( 'orddd_enable_next_day_delivery' ) ) {
                ...
                <?php foreach ( $orddd_weekdays as $n => $day_name ) {
                    ?>
                    <option name="<?php echo $n; ?>" value="<?php echo $n; ?>" <# if ( data.orddd_after_cutoff_weekday == "<?php echo $n; ?>" ) { #> selected <# } #> > <?php echo $day_name; ?> </option> <?php
                } ?>
            </select>
        </td>
        ...
        <td class="minimumOrderDays">
            ...
            if( 'on' == get_option( 'orddd_enable_same_day_delivery' ) || 'on' == get_option( 'orddd_enable_next_day_delivery' ) ) {
                ?>
                <input type="text" value="{{ data.orddd_minimumOrderDays }}" name="orddd_minimumOrderDays[{{ data.row_id }}]" class="orddd_minimumOrderDays" disabled = "disabled" data-attribute="orddd_minimumOrderDays" placeholder="In Hours, eg. 5" />
                <?php
            } else {
                ?>
