...
class orddd_common {
	...
	public static function delivery_date_language( $delivery_date_formatted, $delivery_date_timestamp ) {
		global $orddd_languages, $orddd_languages_locale;
		...
		if( $delivery_date_timestamp != '' ) {
            if( $date_language != 'en-GB' ) {
                ...
                $time = setlocale( LC_ALL, $orddd_languages_locale[ $locale_format ] );
                ...
                if( isset( $year_str ) ) {
                    if ( $time_settings != '00:01' && $time_settings != '00:00' ) {
                        ...
                        if ( $time_format == '1' ) {
                            $time_format_to_show = '%I:%M';
                            $delivery_date_formatted = strftime( $year_str . ' ' . $time_format_to_show, $delivery_date_timestamp ) . " " . ( ( $delivery_date_timestamp%86400 ) < 43200 ? 'AM' : 'PM' );
                        } else {
                            $time_format_to_show = '%H:%M';
                            $delivery_date_formatted = strftime( $year_str . ' ' . $time_format_to_show, $delivery_date_timestamp );
                        }
                    } else {
	...
	public static function orddd_get_order_delivery_date( $order_id ) {
	    ...
	    $delivery_date_for_default_label = array_key_exists( ORDDD_DELIVERY_DATE_FIELD_LABEL, $data );
	    if ( isset( $data[ '_orddd_timestamp' ] ) || isset( $data[ get_option( 'orddd_delivery_date_field_label' ) ] ) || isset( $delivery_date_for_default_label ) ) {
	        ...
	        if ( $delivery_date_timestamp != '' ) {
	            ...
	            if ( array_key_exists( get_option( 'orddd_delivery_date_field_label' ), $data ) ) {
	                ...
	                $delivery_date_timestamp = strtotime( str_replace( ",", " ", $data[ ORDDD_DELIVERY_DATE_FIELD_LABEL ][ 0 ] ) );
	                if ( $delivery_date_timestamp != '' ) {
	                    $delivery_date_formatted = date( $orddd_date_formats[ get_option( 'orddd_delivery_date_format' ) ], $delivery_date_timestamp );
	                    $time_settings = date( "H:i", $delivery_date_timestamp );
	                    if ( $time_settings != '00:01' && $time_settings != '00:00' ) {
	...
	public static function orddd_get_order_timeslot( $order_id, $is_subscription_parent_order = false ) {
    	...
    	if ( isset( $data[ '_orddd_time_slot' ] ) && '' != $data[ '_orddd_time_slot' ] ) {
        	...
        	if( isset( $data[ $field_label ][ 0 ] ) && $data[ $field_label ][ 0 ] != '' ) {
	    	...
	    	foreach( $data as $data_key => $data_value ) {
	    		if( preg_match( "/^([01]?[0-9]|2[0-3]):[0-5][0-9]/", $data_value[0] ) ) {
	    			$order_time_slot = $data_value[0];
	    		}
	    	}
	    }
                ...
                if( isset( $shipping_settings[ 'time_settings' ] ) ) {
                    ...
                    if( isset( $time_settings[ 'from_hours' ] ) && $time_settings[ 'from_hours' ] != 0
                        && isset( $time_settings[ 'to_hours' ] ) && $time_settings[ 'to_hours' ] != 0 ) {
                            ...
                            $var_time = "{\"hourMin\":\"$from_hours\",\"hourMax\":\"$to_hours\",\"stepMinute\":\"5\" $time_show_format}";
	...
	public static function load_hidden_fields( $called_from = '' ) {
	    ...
	    $min_date_array = orddd_common::get_min_date( $delivery_time_seconds, array( 'enabled' => get_option( 'orddd_enable_delivery_time' ), 'from_hours' => get_option( 'orddd_delivery_from_hours' ), 'to_hours' => get_option( 'orddd_delivery_to_hours' ) ), $holidays_str, $lockout_days_str );
	    // check mindate is today.. if yes, then check if all time slots are past, if yes, then set mindate to tomorrow
	    if ( get_option( 'orddd_enable_time_slot' ) == 'on' && get_option( 'orddd_enable_same_day_delivery' ) == '' && get_option( 'orddd_enable_next_day_delivery' ) == '' ) {
	        $last_slot_hrs = 0;
	        $last_slot_min = 00;
	        $current_date  =   date( 'j-n-Y', $current_time );
	        $existing_timeslots_arr = json_decode( get_option( 'orddd_delivery_time_slot_log' ) );
	    ...
	    if ( get_option( 'orddd_delivery_from_hours' ) != '' ) {
	    	if( 'on' != get_option ( 'orddd_enable_same_day_delivery' ) && 'on' != get_option ( 'orddd_enable_next_day_delivery' )  ) {
	    		if( ( get_option( 'orddd_delivery_from_hours' ) < $current_hour || ( get_option( 'orddd_delivery_from_hours' ) == $current_hour && $current_minute > 0 ) )
	            && $current_date == $min_date_array[ 'min_date' ] && ( $current_hour > $min_date_array[ 'min_hour' ] || ( $current_hour == $min_date_array[ 'min_hour' ] && $current_minute > $min_date_array[ 'min_minute' ] ) ) ) {
	                ...
	                $var .= '<input type="hidden" name="orddd_min_minute" id="orddd_min_minute" value="0">';
	    ...
	    $var .= '<input type="hidden" name="orddd_minimumOrderDays" id="orddd_minimumOrderDays" value="' . $min_date_array[ 'min_date' ] . '">';
	    ...
	    $var .= '<input type="hidden" name="orddd_auto_populate_first_available_time_slotvv" id="orddd_auto_populate_first_available_time_slot" value="' . get_option( 'orddd_auto_populate_first_available_time_slot' ) . '">';
	    ...
	    $categories = orddd_common::orddd_get_cart_product_categories( false, $order_id );
	    ...
	    $shipping_classes = orddd_common::orddd_get_cart_shipping_classes( false, $order_id );
	    ...
	    $get_common_delivery_days_json = $delivery_dates_str = $orddd_is_days_common = $orddd_categories_settings_common = $holidays_str = $locked_days_str = '';
		...
		$var .= '<input type=\'hidden\' name=\'orddd_holidays_for_product_category\' id=\'orddd_holidays_for_product_category\' value=\'' . $holidays_str . '\'>';
		...
		$var .= '<input type="hidden" name="orddd_hidden_vars_custom_str" id="orddd_hidden_vars_custom_str" value="' . $hidden_vars_str . '">';
	    ...
	    $var .= '<input type="hidden" id="orddd_partially_booked_dates" class="orddd_partially_booked_dates" value="' . $available_deliveries_arr[0] . '"/>';
	    
	    if( isset( $available_deliveries_arr[1] ) ) {
	    	$var .= '<input type="hidden" id="orddd_available_deliveries" name="orddd_available_deliveries" value="' . $available_deliveries_arr[1] . '">';	
	    }
	...
	public static function orddd_get_zone_id( $order_id = "", $is_ajax = true ) {
		...
		if ( defined( 'WOOCOMMERCE_VERSION' ) && version_compare( WOOCOMMERCE_VERSION, "2.6.0", '>=' ) ) {
			...
			if( class_exists( 'WC_Shipping_Zones' ) ) {
				...
				if( isset( $results[0] ) ) {
					$shipping_zone_id = $results[0]->method_id . ":" . $results[0]->instance_id;
					if( 'table_rate' == $results[0]->method_id ) {
 						if ( is_array( $post_array ) && count( $post_array ) > 0 ) { 
                     	...
                     	$table_rate_shipping_classes = $wpdb->get_results( "SELECT rate_id, rate_class FROM {$wpdb->prefix}woocommerce_shipping_table_rates WHERE shipping_method_id = '{$results[0]->instance_id}'" );
 
                         foreach( $table_rate_shipping_classes as $tkey => $tvalue ) {
                         	if( $shipping_class_id == $tvalue->rate_class ) {
 		                           	$shipping_zone_id = $results[0]->method_id . ":" . $results[0]->instance_id . ":" . $tvalue->rate_id;
 		                    }		                                           
 	                    }
 					} else if( 'fedex' == $results[0]->method_id ) {
 						$matching_zone_id_array = $wpdb->get_results( "SELECT zone_id FROM {$wpdb->prefix}woocommerce_shipping_zone_methods WHERE instance_id = '{$results[0]->instance_id}'" );
	...
	public static function orddd_shipping_methods_for_custom() {
	    ...
	    if ( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && get_option( 'orddd_enable_delivery_date' ) == 'on' ) {
	        $shipping_based_settings_query = "SELECT option_value, option_name FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'orddd_shipping_based_settings_%' AND option_name != 'orddd_shipping_based_settings_option_key' ORDER BY option_id DESC";
	        $results = $wpdb->get_results( $shipping_based_settings_query );
	        $shipping_settings =  array();
	        foreach ( $results as $key => $value ) {
	...
	public static function get_datepicker_options( $holidays_str, $lockout_str ) {
	    ...
	    if ( $time_settings_enable == 'on' ) {
	        ...
	        $delivery_time_seconds = $minimum_delivery_time * 60 * 60;
	        $min_date_array = orddd_common::get_min_date( $delivery_time_seconds, array( 'enabled' => $time_settings_enable, 'from_hours' => get_option( 'orddd_delivery_from_hours' ), 'to_hours' => get_option( 'orddd_delivery_to_hours' ) ), $holidays_str, $lockout_str );
	        if( !is_account_page() && !is_admin() ) {
		        if ( get_option( 'orddd_delivery_from_hours' ) != '' ) {
		        	if( 'on' != get_option ( 'orddd_enable_same_day_delivery' ) && 'on' != get_option ( 'orddd_enable_next_day_delivery' )  ) {
			            if( ( get_option( 'orddd_delivery_from_hours' ) < $current_hour || ( get_option( 'orddd_delivery_from_hours' ) == $current_hour && $current_minute > 0 ) )   
			                 && $current_date == $min_date_array[ 'min_date' ] && ( $current_hour > $min_date_array[ 'min_hour' ] || ( $current_hour == $min_date_array[ 'min_hour' ] && $current_minute > $min_date_array[ 'min_minute' ] ) ) && $current_hour <= get_option( 'orddd_delivery_to_hours' ) ) {
	    ...
	    if ( get_option( 'orddd_enable_next_day_delivery' ) != 'on' && get_option( 'orddd_enable_same_day_delivery' ) != 'on' && get_option( 'orddd_enable_delivery_date' ) == 'on' ) {
	...
	public static function get_disabled_days() {
	    ...
	    if ( get_option( 'orddd_enable_next_day_delivery' ) == 'on' && get_option( 'orddd_enable_delivery_date' ) == 'on' ) {
	        ...
	        $cut_off_timestamp = gmmktime( $cut_off_hour, $cut_off_minute, 0, $current_month, $current_date, $current_year );
            $next_day = date( 'w', $cut_off_timestamp + 86400 );
            $next_day_cut_off = $cut_off_timestamp + 86400;
            ...
            if ( $cut_off_timestamp > $current_time ) {
            	...
            	for ( $j = $next_day; $j <= 6; ) {
                	...
                	if( $is_day_disabled == 'yes' ) {
                        $days_disabled = "Yes";
		                $current_time = strtotime( "+1 day", $current_time );
		                $next_day_cut_off = strtotime( "+1 day", $next_day_cut_off );
                        $j = date( 'w', $next_day_cut_off );
                    } else {
                    	if( $current_time <= $next_day_cut_off ) {
	                    	if( in_array( date( ORDDD_HOLIDAY_DATE_FORMAT, $next_day_cut_off ), $holiday_date_arr ) ) {
	                    		$days_disabled = "Yes";
				    	    	$next_day_cut_off = strtotime( "+1 day", $next_day_cut_off );
				    	    	$current_time = strtotime( "+1 day", $current_time );
	                        	$j = date( 'w', $current_time );  
                ...
                if( $days_disabled == "Yes" ) {
                    $disabled_days[] = date( ORDDD_HOLIDAY_DATE_FORMAT, $next_day_cut_off );
                } else {
                    $disabled_days[] = date( ORDDD_HOLIDAY_DATE_FORMAT, $next_day_cut_off );
                }
            }
	...
	*/
	public static function orddd_restore_deliveries( $order_id, $old_status, $new_status ) {
		if( ( $old_status == 'cancelled' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) || ( $old_status == 'refunded' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) 
	        || ( $old_status == 'trashed' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) ) || ( $old_status == 'failed' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'trashed' ) ) ) {
            	...
            	foreach ( $shipping_settings_to_check as $setting_key => $setting_value ) {
	            	if ( $setting_value[ 'date_lockout' ] != '' && $setting_value[ 'date_lockout' ] != '0' ) {
	                    if( $delivery_date_timestamp != '' ) {
	                        ...
	                        update_option( $value->option_name, $setting_value );
	                    }
	                    $shipping_based_lockout = "Yes";
	                }
	...
	public static function orddd_admin_product_has_delivery( $product_id ) {
	    ...
	    if( $terms == '' ) {
	        ...
	        foreach ( $terms as $term ) {
	           ...
	           $delivery_date  = get_woocommerce_term_meta( $categoryid, 'orddd_delivery_date_for_product_category', true );
	           if ( has_filter( 'orddd_remove_delivery_date_if_product_category_no' ) ) {
	               $is_enabled = apply_filters( 'orddd_remove_delivery_date_if_product_category_no', $is_enabled );
	           }
	           if ( $is_enabled == 'yes' ) {
	            ...
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	            	if( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
		                ...
		                foreach( $shipping_methods as $skey => $sval ) {
		                	$unique_settings_key[ $sval ] = 'custom_settings_' . $j;
		                }
        			}
        			$shipping_method_js_str[ $i ][ 'unique_settings_key' ]             = $unique_settings_key;
	            	...
	            	if( isset( $shipping_settings[ 'product_categories' ] ) ) {
		                ...
		                foreach( $product_categories as $skey => $sval ) {
		                	$unique_settings_key[ $sval ] = 'custom_settings_' . $j;
		                }
        			}
        			$shipping_method_js_str[ $i ][ 'unique_settings_key' ]             = $unique_settings_key;
	...
	public static function orddd_get_shipping_hidden_variables( $shipping_settings, $timeslot_field_mandatory ) {
        ...
        if( isset( $shipping_settings[ 'time_settings' ] ) ) {
            ...
            if( isset( $time_settings[ 'from_hours' ] ) && $time_settings[ 'from_hours' ] != 0
                ...
                if( is_array( $from_hour_values ) && count( $from_hour_values ) ) {
                	...
                	$from_minutes = $from_hour_values[ 'from_minutes' ];
                }

                $to_hours = $time_settings[ 'to_hours' ];

                $new_array[ "orddd_min_hour" ] = $from_hours;
                $new_array[ "orddd_min_minute" ] = $from_minutes;
        ...
        $same_day_cut_off = orddd_common::orddd_get_highest_same_day();
        $same_day = array();
	    ...
	    //Nexy Day Delivery
	    $next_day_cut_off = orddd_common::orddd_get_highest_next_day();
        $next_day = array();
        ...
        $disabled_days_str = ( is_admin() ) ? '' : orddd_common::orddd_get_shipping_disabled_days_str( $shipping_settings, $same_day, $next_day );
        $new_array[ "orddd_disabled_days_str" ] = $disabled_days_str;

        //Minimum Delivery Time
        $minimum_time  = orddd_common::orddd_get_higher_minimum_delivery_time();
        ...
        } else if( !in_array( "orddd_weekday_" . $current_weekday, $weekdays_to_check ) && ( !isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) || ( isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) && 'on' == $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] && is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && !in_array( $date_to_check, $all_specific_dates ) ) ) ) {
            $current_time = strtotime( $current_date );
        }
        
        $delivery_time_seconds = $minimum_delivery_time * 60 * 60;

        $min_date_array = orddd_common::get_min_date( $delivery_time_seconds, array( 'enabled' => $time_slider_enabled, 'from_hours' => $from_hours, 'to_hours' => $to_hours ), $holidays_str, $lockout_days_str, $shipping_settings );

        // check mindate is today.. if yes, then check if all time slots are past, if yes, then set mindate to tomorrow
        if ( isset( $shipping_settings[ 'time_slots' ] ) && $shipping_settings[ 'time_slots' ] != '' && ( !isset( $shipping_settings[ 'same_day' ] ) || ( isset( $shipping_settings[ 'same_day' ] ) && isset( $shipping_settings[ 'same_day' ][ 'after_hours' ] ) && $shipping_settings[ 'same_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'same_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'same_day' ][ 'after_minutes' ] == 00 ) ) && ( !isset( $shipping_settings[ 'next_day' ] ) || ( isset( $shipping_settings[ 'next_day' ] ) && isset( $shipping_settings[ 'next_day' ][ 'after_hours' ] ) && $shipping_settings[ 'next_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'next_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'next_day' ][ 'after_minutes' ] == 00 ) ) ) {
			...
			if( $last_slot_hrs != 0 ) {
				...
				if ( $difference->invert == 0 || $diff_hour_in_seconds < $delivery_time_seconds ) {
                	$min_date_array[ 'min_date' ] = date( 'j-n-Y', strtotime( $min_date_array[ 'min_date' ] . '+1 day' ) );
                }
            }
        }
            ...
            if( isset( $subscrition_var[ 'orddd_start_date_for_subscription' ] ) ) {
                ...
                if( isset( $shipping_settings[ 'minimum_delivery_time' ] ) ) {
                    ...
                    if( $shipping_settings[ 'minimum_delivery_time' ] != 0 && $shipping_settings[ 'minimum_delivery_time' ] != '' ) {
                        $min_date_array = orddd_common::get_min_date( $delivery_time_seconds, array( 'enabled' => $time_slider_enabled, 'from_hours' => $from_hours, 'to_hours' => $time_settings[ 'to_hours' ] ), $holidays_str, $lockout_days_str, $shipping_settings );
                    }    
                }
	...
	public static function orddd_get_shipping_time_settings_variable( $shipping_settings, $holidays_str, $lockout_str ) {
		...
		if( isset( $shipping_settings[ 'time_settings' ] ) ) {
            ...
            if( isset( $time_settings[ 'from_hours' ] ) && $time_settings[ 'from_hours' ] != 0
	            ...
	            $var_time = "{\"hourMin\":\"$from_hours\",\"minuteMin\":\"$from_minutes\",\"hourMax\":\"$to_hours\",\"stepMinute\":\"5\" $time_show_format}";  
            } 
        }
	    return $var_time;
	}
	...
	public static function orddd_get_shipping_from_time( $time_settings, $shipping_settings, $holidays_str, $lockout_str ) {
        ...
        $current_hour          = date( "H", $current_time );
        $current_minute        = date( "i", $current_time );
        $delivery_time_seconds = intval( $shipping_settings[ 'minimum_delivery_time' ] ) * 60 * 60;
        $min_date_array        = orddd_common::get_min_date( $delivery_time_seconds,  array( 'enabled' => 'on', 'from_hours' => $time_settings[ 'from_hours' ], 'to_hours' => $time_settings[ 'to_hours' ] ), $holidays_str, $lockout_str, $shipping_settings );
        
        if ( $time_settings[ 'from_hours' ] != '' ) {
        	if( ( !isset( $shipping_settings[ 'same_day' ] ) || ( isset( $shipping_settings[ 'same_day' ] ) && isset( $shipping_settings[ 'same_day' ][ 'after_hours' ] ) && $shipping_settings[ 'same_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'same_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'same_day' ][ 'after_minutes' ] == 00 ) ) && ( !isset( $shipping_settings[ 'next_day' ] ) || ( isset( $shipping_settings[ 'next_day' ] ) && isset( $shipping_settings[ 'next_day' ][ 'after_hours' ] ) && $shipping_settings[ 'next_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'next_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'next_day' ][ 'after_minutes' ] == 00 ) ) ) { 
                if( ( $time_settings[ 'from_hours' ] < $current_hour || ( $time_settings[ 'from_hours' ] == $current_hour && $current_minute > 0 ) )
                    ...
                    $from_hour_values[ 'from_minutes' ] = 0;
                }
            } else {
					if( $time_settings[ 'from_hours' ] < $current_hour || ( $time_settings[ 'from_hours' ] == $current_hour && $current_minute > 0 ) ) {
                    $from_hour_values[ 'from_hours' ] = $current_hour;
                ...
                if ( $cut_off_timestamp > $current_time ) {
                    ...
                    for ( $j = $current_weekday; $j <= 6; ) {
                    	...
                    	if( get_option( 'orddd_enable_shipping_days' ) == 'on' ) {
                    		...
                    		if( ( is_array( $orddd_weekdays_enabled ) && count( $orddd_weekdays_enabled ) > 0 && !isset( $orddd_weekdays_enabled[ 'orddd_weekday_' . $j ] ) ) && ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && !in_array( date( ORDDD_HOLIDAY_DATE_FORMAT, $current_time ), $all_specific_dates ) ) ) {
								$is_day_disabled = 'yes';
                        	}
                    	}
	...
	public static function orddd_common_delivery_days_for_product_category( $orddd_shipping_id, $is_ajax = false ) {
		...
		if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
			...
			$specific_dates            = array();
			$holidays                  = array();
			$categories                = orddd_common::orddd_get_cart_product_categories( $is_ajax, '' );
			$shipping_classes          = orddd_common::orddd_get_cart_shipping_classes( $is_ajax, '' ); 
			...
			if( is_array( $settings_to_compare ) && count( $settings_to_compare ) == 0 ) {
				foreach( $shipping_classes as $sckey => $scvalue ) {
					if( is_array( $shipping_settings_results ) && count( $shipping_settings_results ) > 0 ) {
						foreach ( $shipping_settings_results as $skey => $svalue ) {
							$shipping_settings = get_option( $svalue->option_name );
							if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][0] ) && $shipping_settings[ 'delivery_settings_based_on' ][0] == 'shipping_methods' && isset( $shipping_settings[ 'shipping_methods' ] ) ) {
		...
		return array( 'orddd_common_weekdays' => $common_delivery_days, 'orddd_common_specific_dates' => $common_delivery_dates, 'orddd_common_holidays' => $all_holidays, 'orddd_common_locked_days' => $locked_days, 'orddd_is_days_common' => $is_days_common, 'orddd_categories_settings_common' => $is_common );
	}
	...
	public static function orddd_get_common_delivery_dates( $specific_dates, $common_days, $delivery_days ) {
		...
		if( is_array( $specific_dates ) && count( $specific_dates ) > 0 ) {
			$i = 0; $j = 1;
			if( ( is_array( $specific_dates ) && count( $specific_dates ) == 1 ) && ( is_array( $delivery_days ) && count( $delivery_days ) == 0 ) ) {
	...
	public static function orddd_get_shipping_settings() {
		global $wpdb;
		$shipping_based_settings_query = "SELECT option_value, option_name FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'orddd_shipping_based_settings_%' AND option_name != 'orddd_shipping_based_settings_option_key' ORDER BY option_id DESC";
		$results = $wpdb->get_results( $shipping_based_settings_query );
		return $results;
	...
	public static function orddd_get_cart_product_categories( $is_ajax, $order_id ) {
		global $woocommerce;
		$categories = array();
		if( !is_admin() || ( is_admin() && true == $is_ajax ) ) {
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
	...
	public static function orddd_get_cart_shipping_classes( $is_ajax, $order_id ) {
		global $woocommerce;
	 ...
	 * 
	 * @param int $order_id Order ID
	 * @return int Highest minimum delivery time
	 * @since 6.2
	 */
	public static function orddd_get_higher_minimum_delivery_time( $order_id = '' ) {
    	                    ...
    	                    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
    	                        foreach ( $results as $key => $value ) {
    	                            ...
    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'product_categories' ) {
    	                                if ( isset( $shipping_settings[ 'product_categories' ] ) ) {
    	                                    $product_category = $shipping_settings[ 'product_categories' ];
    	                                    if ( in_array( $val->slug, $product_category ) && $minimum_delivery_time < $shipping_settings[ 'minimum_delivery_time' ] && $shipping_settings[ 'minimum_delivery_time' ] != '' ) {
    	                                    	$minimum_delivery_time = $shipping_settings[ 'minimum_delivery_time' ];
    	                                    }
    	                                }
    	                            }
	    	                    ...
	    	                    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
	    	                        foreach ( $results as $key => $value ) {
	    	                            ...
	    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	    	                                if ( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
	    	                                    $shipping_methods = $shipping_settings[ 'shipping_methods' ];
	    	                                    if ( in_array( $val->slug, $shipping_methods ) && $minimum_delivery_time < $shipping_settings[ 'minimum_delivery_time' ] && $shipping_settings[ 'minimum_delivery_time' ] != '' ) {
	    	                                    	$minimum_delivery_time = $shipping_settings[ 'minimum_delivery_time' ];
	    	                                    }
	    	                                }
	    	                            }
	 ...
	 * 
	 * @param int $order_id Order ID
	 * @return string Highest same day cut-off time
	 * @since 7.5
	 */

	public static function orddd_get_highest_same_day( $order_id = '' ) {
    	global $post, $wpdb, $woocommerce;
	    $same_day = array();
	    $same_day_hours = 0;
	    $same_day_min = 00;
    	                        ...
    	                        foreach ( $results as $key => $value ) {
    	                            ...
    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'product_categories' ) {
    	                                if ( isset( $shipping_settings[ 'product_categories' ] ) && !isset( $shipping_settings[ 'shipping_methods_for_categories' ] ) ) {
    	                                    $product_category = $shipping_settings[ 'product_categories' ];
    	                                    if ( in_array( $val->slug, $product_category ) && isset( $shipping_settings[ 'same_day' ] ) && ( $same_day_hours < $shipping_settings[ 'same_day' ][ 'after_hours' ] || ( $same_day_hours == $shipping_settings[ 'same_day' ][ 'after_hours' ] && $same_day_min == $shipping_settings[ 'same_day' ][ 'after_minutes' ] ) ) ) {
    	                                    	$same_day = $shipping_settings[ 'same_day' ];
    	                    	...
    	                    	if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
    	                        	foreach ( $results as $key => $value ) {
	    	                            ...
	    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	    	                                if ( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
	    	                                    $shipping_methods = $shipping_settings[ 'shipping_methods' ];
	    	                                    if ( in_array( $val->slug, $shipping_methods ) && isset( $shipping_settings[ 'same_day' ] ) && ( $same_day_hours < $shipping_settings[ 'same_day' ][ 'after_hours' ] || ( $same_day_hours == $shipping_settings[ 'same_day' ][ 'after_hours' ] && $same_day_min == $shipping_settings[ 'same_day' ][ 'after_minutes' ] ) ) ) {
	    	                                    	$same_day = $shipping_settings[ 'same_day' ];
	    	                                    	$same_day_hours = $shipping_settings[ 'same_day' ][ 'after_hours' ];
	    	                                    	$same_day_min = $shipping_settings[ 'same_day' ][ 'after_minutes' ];
	    	                                    }
	 ...
	 * 
	 * @param int $order_id Order ID
	 * @return string Highest next day cut-off time
	 * @since 7.5
	 */
	public static function orddd_get_highest_next_day( $order_id = '' ) {
    	global $post, $wpdb, $woocommerce;
	    $next_day = array();
	    $next_day_hours = 0;
	    $next_day_min = 00;
    	                    ...
    	                    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
    	                        foreach ( $results as $key => $value ) {
    	                            ...
    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'product_categories' ) {
    	                                if ( isset( $shipping_settings[ 'product_categories' ] ) ) {
    	                                    $product_category = $shipping_settings[ 'product_categories' ];
    	                                    if ( in_array( $val->slug, $product_category ) && isset( $shipping_settings[ 'next_day' ] ) && ( $next_day_hours < $shipping_settings[ 'next_day' ][ 'after_hours' ] || ( $next_day_hours == $shipping_settings[ 'next_day' ][ 'after_hours' ] && $next_day_min == $shipping_settings[ 'next_day' ][ 'after_minutes' ] ) ) ) {
    	                                    	$next_day = $shipping_settings[ 'next_day' ];
    	                    	...
    	                    	if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
    	                        	foreach ( $results as $key => $value ) {
	    	                            ...
	    	                            if ( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	    	                                if ( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
	    	                                    $shipping_methods = $shipping_settings[ 'shipping_methods' ];
	    	                                    if ( in_array( $val->slug, $shipping_methods ) && isset( $shipping_settings[ 'next_day' ] ) && ( $next_day_hours < $shipping_settings[ 'next_day' ][ 'after_hours' ] || ( $next_day_hours == $shipping_settings[ 'next_day' ][ 'after_hours' ] && $next_day_min == $shipping_settings[ 'next_day' ][ 'after_minutes' ] ) ) ) {
	    	                                    	$next_day = $shipping_settings[ 'next_day' ];
	    	                                    	$next_day_hours = $shipping_settings[ 'next_day' ][ 'after_hours' ];
	    	                                    	$next_day_min = $shipping_settings[ 'next_day' ][ 'after_minutes' ];
	    	                                    }
	 ...
	 * @param int $order_id Order ID
	 *
	 * @return string Shipping class
	 * 
	 * @since 3.7
	 */
	public static function orddd_get_product_shipping_class( $order_id = '' ) {
	    $shipping_class_to_send = $shipping_class = '';
	    if( '' == $order_id ) {
	        global $woocommerce, $wpdb;
	        $categories = array();
	        $shipping_class_to_send = '';
	        $sc_for_higher_min_time = orddd_common::orddd_get_shipping_class_for_higher_minimum_delivery_time();
	        foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
	            ...
	            if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
                    foreach ( $results as $key => $value ) {
                        ...
                        if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
                            if( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
                                $shipping_methods = $shipping_settings[ 'shipping_methods' ];
                                if( in_array( $shipping_class, $shipping_methods ) && $shipping_class == $sc_for_higher_min_time ) {
                                    $shipping_class_to_send = $shipping_class;
                                    if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) ) {
                                        break 2;
                                    }
                        	...
                        	if( isset( $shipping_settings[ 'shipping_methods_for_categories' ] ) ) {
                                $shipping_methods_for_categories = $shipping_settings[ 'shipping_methods_for_categories' ];
                                if( in_array( $shipping_class, $shipping_methods_for_categories ) && $shipping_class == $sc_for_higher_min_time ) {
                                    $shipping_class_to_send = $shipping_class;
                                    if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) ) {
                                        break 2;
                                    }
	        ...
	        $order = new WC_Order( $order_id );
	        $items = $order->get_items();
	        $sc_for_higher_min_time = orddd_common::orddd_get_shipping_class_for_higher_minimum_delivery_time( $order_id );
	        foreach( $items as $key => $value ) {
				$product_id = $value[ 'product_id' ];
	            $terms = get_the_terms( $product_id , 'product_shipping_class' );
	                    ...
	                    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
	                        foreach ( $results as $key => $value ) {
	                            ...
	                            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	                                if( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
	                                    $shipping_methods = $shipping_settings[ 'shipping_methods' ];
	                                    if( in_array( $val->slug, $shipping_methods ) && $val->slug == $sc_for_higher_min_time ) {
	                        		...
	                        		if( isset( $shipping_settings[ 'shipping_methods_for_categories' ] ) ) {
	                                	$shipping_methods_for_categories = $shipping_settings[ 'shipping_methods_for_categories' ];
	                                	if( in_array( $val->slug, $shipping_methods_for_categories ) && $val->slug == $sc_for_higher_min_time ) {
	                                		$shipping_class_to_send = $val->slug;
	                                        if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) ) {
	                                            break 3;
	                                        }
	...
	public static function orddd_get_total_orders_to_export() {
	    ...
	    foreach ( $results as $key => $value ) {
	        ...
	        if ( isset( $data[ $date_field_label ][ 0 ] ) ) {
	            $delivery_date_timestamp = strtotime( str_replace( ",", " ", $delivery_date_formatted ) );
	        }
	        
	        if ( isset( $data[ ORDDD_DELIVERY_DATE_FIELD_LABEL ][ 0 ] ) ) {
	            $default_delivery_date_timestamp = strtotime( str_replace( ",", " ", $delivery_date_formatted ) );
	        }
	...
	public static function orddd_get_all_future_orders() {
		global $wpdb, $orddd_date_formats;
	    ...
	    $orddd_query = "SELECT ID, post_status FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ('wc-cancelled', 'wc-refunded', 'trash', 'wc-failed') AND ID IN ( SELECT post_id FROM `" . $wpdb->prefix . "postmeta` WHERE ( meta_key = '_orddd_timestamp' AND meta_value >= '" . $current_time . "' ) )";
	 ...
	 * @since 4.1
	 */
	public static function orddd_get_total_product_quantities() {
	    global $woocommerce;
	...
	public static function get_timeslot_values( $time_slot ) {
	    $time_slot_values = $allpos = $delivery_days_arr = array();
	    $time_slot_selected = '';
	    $offset = 0;
	    $allpos = array();
            ...
            if( "Yes" == $shipping_based ) {
            	if( is_array( $custom_settings ) && count( $custom_settings ) > 1 ) {
            		foreach( $custom_settings as $skey => $sval ) {
            			if( isset( $sval[ 'time_slots' ] ) && ( isset( $sval[ 'is_combination_enabled' ] ) && 'yes' == $sval[ 'is_combination_enabled' ] ) ) {
					...
					if( count( $time_slots ) > 0 ) {
						$min_lockout = min( array_column( $time_slots, 'lockout' ) );	
					}
            	...
            	if( is_array( $time_slots ) && count( $time_slots ) > 0 ) {
		            foreach( $time_slots as $tk => $tv ) {
	                    ...
	                    if( is_array( $tv[ 'selected_days' ] ) ) {
	                        if ( $tv[ 'delivery_days_selected' ] == 'specific_dates' ) {
	                        ...
	                        } else if ( $tv[ 'delivery_days_selected' ] == 'weekdays' ) {
	                            foreach( $tv[ 'selected_days' ] as $dkey => $dval ) {
	                                if( $min_lockout != 0 ) {
	                            		$arr1[ $dval ][ $time_slot_val ] = $min_lockout;	
	                            	} else {
	...
	*/ 
	...
	public static function orddd_get_timeslot_charges( $time_slot ) {
		global $wpdb;
		$timeslot_charges = '';
		$shipping_based = "No";
	    ...
	    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
	        ...
	        $custom_settings_to_load = orddd_common::orddd_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location );
                ...
                foreach( $time_slots as $tk => $tv ) {
                	if( $tv != '' ) {
                        ...
                        if( $time_slot == $time_slot_val ) {
                            $time_slot_str = str_replace( '}', '', $tv );
                            $time_slot_str = str_replace( '{', '', $time_slot_str );
						    
						    $time_slot_charges_lable = strrchr( $time_slot_str, ":" );
						    $time_slot_charges_lable_str = substr( $time_slot_charges_lable, 1 );
						    if( '' == $time_slot_charges_lable_str ) {
						    	$time_slot_charges_lable_str = "Time Slot Charges";	
						    }
						    $time_slot_charges_lable_str_length = strlen( $time_slot_charges_lable );
						    							    
						    $time_slot_charges_string = substr( $time_slot_str, 0, -( $time_slot_charges_lable_str_length ) );
						    $time_slot_charges_str = strrchr( $time_slot_charges_string, ":" );
						    $time_slot_charges_str_length = strlen( $time_slot_charges_str );
						    $timeslot_charges = substr( $time_slot_charges_str, 1, $time_slot_charges_str_length );
						}
	                }
                }
            }
		...
		if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
	    	...
	    	if ( isset( $orddd_calc_taxes) && 'yes' == $orddd_calc_taxes ) {
	...
	public static function get_min_date( $delivery_time_seconds, $delivery_time_enabled, $holidays_str, $lockout_str, $shipping_settings = array() ) {		
	    global $orddd_shipping_days, $orddd_weekdays;
            ...
            if( "checked" != get_option( 'orddd_shipping_day_' . $current_weekday ) || in_array( $date_to_check, $holidays ) || in_array( $date_to_check, $bookedDays ) && $is_all_shipping_days_disabled == 'No' ) {
                $current_time = strtotime( $current_date );
        			...
        			( ( "checked" != get_option( 'orddd_weekday_' . $current_weekday ) && 
        			  ...
        			  ( "checked" != get_option( 'orddd_weekday_' . $current_weekday ) && 
        			  'on' != get_option( 'orddd_enable_specific_delivery_dates' ) ) || 
        			  in_array( $date_to_check, $holidays ) || 
        ...
        // Min date should be calculated if same day and/or next day delivery is NOT enabled either in General Settings or custom settings
        if( is_array( $shipping_settings ) && count( $shipping_settings ) > 0 && ( !isset( $shipping_settings[ 'same_day' ] ) || ( isset( $shipping_settings[ 'same_day' ] ) && isset( $shipping_settings[ 'same_day' ][ 'after_hours' ] ) && $shipping_settings[ 'same_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'same_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'same_day' ][ 'after_minutes' ] == 00 ) ) && ( !isset( $shipping_settings[ 'next_day' ] ) || ( isset( $shipping_settings[ 'next_day' ] ) && isset( $shipping_settings[ 'next_day' ][ 'after_hours' ] ) && $shipping_settings[ 'next_day' ][ 'after_hours' ] == 0 && isset( $shipping_settings[ 'next_day' ][ 'after_minutes' ] ) && $shipping_settings[ 'next_day' ][ 'after_minutes' ] == 00 ) ) ) {
        	$calculate_min = true;
        } else if( is_array( $shipping_settings ) && count( $shipping_settings ) == 0 && get_option( 'orddd_enable_same_day_delivery' ) != 'on' && get_option( 'orddd_enable_next_day_delivery' ) != 'on' ) {
        	$calculate_min = true;
		}
        
        // Min Date calculation
        if( $calculate_min && $delivery_time_seconds != 0 && $delivery_time_seconds != '' ) {
                ...
                if( $i >= 0 ) {
            		...
            		if ( 'yes' == $weekday_disabled && 'No' == $is_all_disable_weekdays ) {
						...
						if( $current_time_check <= $cut_off_date_time ) {
							...
							if ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && 'Yes' == $is_all_disable_weekdays ) {
							...
							if( $orddd_disable_for_holidays != 'yes' ) {
								if( in_array( $m . '-' . $d . '-' . $y, $holidays ) ) {	
									$cut_off_date_time = strtotime( "+1 day", $cut_off_date_time );
	                        		$cut_off_timestamp = strtotime( "+1 day", $cut_off_timestamp );
								}
							}
	        ...
	        if( isset( $delivery_time_enabled[ 'enabled' ] ) && 'on' == $delivery_time_enabled[ 'enabled' ] ) {
	        	if( isset( $delivery_time_enabled[ 'to_hours' ] ) && $delivery_time_enabled[ 'to_hours' ] < date( "H", $cut_off_timestamp ) ) {
	        		$cut_off_date_time = strtotime( "+1 day", $cut_off_date_time );
	        		$cut_off_time_date = date( 'd-m-Y', $cut_off_timestamp );
                    $cut_off_timestamp = strtotime( $cut_off_time_date . ' ' . $delivery_time_enabled[ 'from_hours' ] . ":00" );
	           	}
	        }
	    	...
	    	if( $calculate_min ) {
	                ...
	                if( $i >= 0 ) {
	            		...
	            		if ( 'yes' == $weekday_disabled && 'No' == $is_all_disable_weekdays ) {
							...
							if( $current_timestamp <= $current_time ) {
								...
								if ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 ) {
		                            if( !in_array( $m . "-" . $d . "-" . $y, $all_specific_dates ) && 'Yes' == $is_all_disable_weekdays ) {
		                                $current_time = strtotime( "+1 day", $current_time );
		                            } else if ( $delivery_day_3 != '' ) {
		                                if( $delivery_day_3 != $current_time && $current_time < $delivery_day_3 ) {
		                                    $current_time = strtotime( "+1 day", $current_time );
		        ...
		        if( 'on' == isset( $delivery_time_enabled[ 'enabled' ] ) && 'on' == $delivery_time_enabled[ 'enabled' ] ) {
		        	if( isset( $delivery_time_enabled[ 'to_hours' ] ) && $delivery_time_enabled[ 'to_hours' ] < date( "H", $current_time ) ) {
		        		$current_time = strtotime( "+1 day", $current_time );
		        		$current_date = date( "j-n-Y", $current_time );
		        		$current_time_date = date( 'd-m-Y', $current_time );
	                    $current_date_timestamp = strtotime( $current_time_date . ' ' . $delivery_time_enabled[ 'from_hours' ] . ":00" );
	 ...
	 * @globals resource $wpdb WordPress object
	 * 
	 * @return int Minimum Delivery Time in hours
	 * @since 5.2
	 */
	public static function get_minimum_delivery_time() {
	    global $wpdb;
	    $shipping_based = "No";
	    $min_hour = 0;
	    ...
	    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
	        $shipping_method = $shipping_class = $location = '';
	        $shipping_settings = array();
	        $shipping_settings_to_check = array(); 
	        $product_categories = array();
            ...
            if( "Yes" == $shipping_settings_exists ) {
            	$same_day_enabled = $next_day_enabled = "No";
		        ...
		        if( $minimum_time != '' && $minimum_time != 0 ) {
		        	...
		        	if( isset( $custom_settings[ 'minimum_delivery_time' ] ) && $custom_settings[ 'minimum_delivery_time' ] != '' && "No" == $same_day_enabled && "No" == $next_day_enabled  ) {
			        	$min_hour = $custom_settings[ 'minimum_delivery_time' ];
			            if( '' == $min_hour ) {
			            	$min_hour = 0;
			            }
			        }   	
	    ...
	    if( "No" == $shipping_based && '' != get_option( 'orddd_minimumOrderDays' ) && 'on' != get_option( 'orddd_enable_same_day_delivery' ) && 'on' != get_option( 'orddd_enable_next_day_delivery' ) ) {
	        $min_hour = get_option( 'orddd_minimumOrderDays' );
	    }
	    ...
	    if( 'on' == get_option( 'orddd_enable_day_wise_settings' ) ) {
            ...
            foreach( $advance_settings as $ak => $av ) {
            	if( $current_weekday == $av[ 'orddd_weekdays' ] ) {
            		if( '' != $av[ 'orddd_minimumOrderDays' ] ) {
            			$min_hour = $av[ 'orddd_minimumOrderDays' ];
            		}
            	}
            }
	    }
	...
	public static function orddd_get_timeslot_display( $time_slot_for_order, $time_slot_for_multiple ) {
	    ...
	    if( isset( $_POST [ 'current_date_to_check' ] ) && $_POST [ 'current_date_to_check' ] == $current_day ) {
	    	$today            =   date( "Y-m-d G:i", $current_time );	
	    } else {
	    	if( isset( $_POST [ 'current_date_to_check' ] ) ) {
	    ...
	    if ( class_exists( 'WC_Subscriptions' ) && get_option( 'orddd_enable_woo_subscriptions_compatibility' ) == 'on' ) {
	    	if ( class_exists( 'ws_addon_for_orddd' ) ) {
        		$subscrition_var = ws_addon_for_orddd::orddd_check_the_subscription_period( 'check_for_time_slot' );
        	}
	    }
	    ...
	    $min_hour = orddd_common::get_minimum_delivery_time();
	    $min_hour_in_sec = $min_hour * 60 * 60;
	    ...
	    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
	        ...
	        $custom_settings = orddd_common::orddd_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location );
	        if( is_array( $custom_settings ) && count( $custom_settings ) > 0 ) {
	        	$shipping_settings_exists = "Yes";
				if( is_array( $custom_settings ) && count( $custom_settings ) == 1 ) {
            		foreach( $custom_settings as $skey => $sval ) {
        ...
        if( 'Yes' == $shipping_settings_exists ) {
        	...
        	$min_date_array = orddd_common::get_min_date( $min_hour_in_sec, array( 'enabled' => $time_slider_enabled, 'from_hours' => $from_hours, 'to_hours' => $to_hours ), $holidays_str, $lockout_str, $custom_settings );
        } else {
		...
		if( isset( $_POST [ 'current_date' ] ) && $_POST [ 'current_date' ] == $current_day ) {
        ...
        if( "Yes" == $shipping_settings_exists ) {
	        ...
	        if( array_key_exists( $delivery_date, $arr1 ) &&  get_option( 'orddd_enable_specific_delivery_dates' ) == 'on' ) {
	            $arr3 = $arr1[ $delivery_date ];
	        } else if( array_key_exists( "orddd_weekday_" . $dw, $arr1 ) || array_key_exists( "all", $arr1 ) ) {
        ...
        foreach( $arr3 as $key => $lockout ) {
            ...
            if ( $lockout != '' && $lockout != '0' && $lockout != ':' ) {
                if ( isset( $arr2[ $current_date ][ $key ] ) && $arr2[ $current_date ][ $key ] >= $lockout ) {
                    ...
                    if( ( is_array( $subscription_var ) && count( $subscription_var ) > 0 && isset( $subscription_var[ 'orddd_if_renewal_subscription' ] ) && $subscription_var[ 'orddd_if_renewal_subscription' ] == 'yes' ) ) {
	...
	public static function orddd_update_delivery_charges( $order_id = '', $delivery_date, $time_slot ) {
	    ...
	    $delivery_charges = $same_day_delivery_charges = $next_day_delivery_charges = $time_slot_charges = $time_slot_charges = 'no';
		...
		foreach( $results_order_item_ids as $key => $value ) {
	        ...
	        if( $value->order_item_type == 'fee' && $value->order_item_name == $charges_arr[ 'charges_label' ] && $value->order_id == $order_id ) {
	            ...
	            if( $charges_arr[ 'fees' ] > 0 && 'on' == $ordd_get_day_wise_setting ) {
	    ...
	    if( $delivery_charges == 'no' && 'on' == $ordd_get_day_wise_setting ) {
                ...
                if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 && get_option( 'orddd_enable_specific_delivery_dates' ) == "on" ) {
                    ...
                    if ( is_array( $dd ) && count( $dd ) > 0 ) {
                        foreach( $dd as $dkey => $dval ) {
                            ...
                            if( $time_slot_key == $previous_timeslot_selected && $delivery_date == $previous_charges_labels[ 'previous_order_date_check' ] ) {
	...
	public static function orddd_get_event_details( $order_id ) {
		...
		$data = get_post_meta( $order_id );
        $order = new WC_Order( $order_id );
        $timeslot_field_label = get_option( 'orddd_delivery_timeslot_field_label' );
        $date_field_label = get_option( 'orddd_delivery_date_field_label' );
        $orddd = new stdClass();
        if ( isset( $data[ '_orddd_timestamp' ][ 0 ] ) && $data[ '_orddd_timestamp' ][ 0 ] != '' && $data[ '_orddd_timestamp' ][ 0 ] >= $current_time ) {
	 ...
	 * @since 7.5
	 */
	public static function orddd_get_delivery_date_field_label( $shipping_method, $product_category, $shipping_class, $location ) {
		global $wpdb;
		...
		if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
			...
			foreach ( $results as $key => $value ) {
                ...
                if( in_array( $shipping_method, $shipping_methods ) || in_array( $shipping_class, $shipping_methods ) ) {
                	if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) && isset( $shipping_settings[ 'orddd_shipping_based_delivery_date_field_label' ] ) && $shipping_settings[ 'orddd_shipping_based_delivery_date_field_label' ] != '' ) {
            			$delivery_date_field_label = $shipping_settings[ 'orddd_shipping_based_delivery_date_field_label' ];
            		}
                }
            }
		}
	 ...
	 * @since 7.5
	 */
	public static function orddd_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location ) {
		global $wpdb;
		...
		if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0) {
			...
			foreach ( $results as $key => $value ) {
                ...
                if( in_array( $shipping_method, $shipping_methods ) || in_array( $shipping_class, $shipping_methods ) ) {
                	if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) && isset( $shipping_settings[ 'orddd_shipping_based_delivery_timeslot_field_label' ] ) && $shipping_settings[ 'orddd_shipping_based_delivery_timeslot_field_label' ] != '' ) {
                    	$delivery_time_field_label = $shipping_settings[ 'orddd_shipping_based_delivery_timeslot_field_label' ];
                    }
                }
            }
		}
	...
	public static function orddd_get_order_shipping_method( $order_id ) {
		global $wpdb;
		$shipping_method = "";
		$query = "SELECT meta_key, meta_value FROM `" . $wpdb->prefix . "woocommerce_order_itemmeta` WHERE order_item_id IN ( SELECT order_item_id FROM `" . $wpdb->prefix . "woocommerce_order_items` WHERE order_item_type = 'shipping' AND order_id = " . $order_id . " )";
    		...
    		if( '' != $zone_id ) {
	            if( false !== strpos( $shipping_method, 'usps' ) ) {
	            	...
	            	if( '' == $option_settings[ 'calculation_type' ] ) {
                    	$shipping_method = $shipping_method . ":" . $zone_id;
                    } else {                   
	...
	public static function orddd_get_order_product_category( $order_id ) {
		$category_to_send = "";
		$order = new WC_Order( $order_id );
        $items = $order->get_items();
        $product_category_for_minimum = orddd_common::orddd_get_product_category_for_higher_minimum_delivery_time( $order_id );
        foreach( $items as $key => $value ) {
            $product_id = $value[ 'product_id' ];
            $terms = get_the_terms( $product_id , 'product_cat' );
            if( $terms != '' ) {
                        ...
                        foreach ( $results as $key => $value ) {
                            ...
                            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'product_categories' ) {
                                if( isset( $shipping_settings[ 'product_categories' ] ) ) {
                                    ...
                                    // Get the Base product category ID
                                    $base_cat_id = orddd_common::get_base_product_category( $val->term_id );
                                    // Get the Slug for the same
                                    $base_cat_slug = orddd_common::ordd_get_cat_slug( $base_cat_id );
                                    if( in_array( $base_cat_slug, $product_categories ) && $base_cat_slug == $product_category_for_minimum ) {
                                        $category_to_send = $base_cat_slug;
                                        if( isset( $shipping_settings[ 'enable_shipping_based_delivery' ] ) ) {
                                            break 3;
                                        }
                                    }
	 ...
	 * @since 8.0
	 */
	public static function orddd_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location = '' )  {
	 	global $wpdb;
