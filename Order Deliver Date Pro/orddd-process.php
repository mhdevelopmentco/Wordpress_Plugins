...
class orddd_process {
    ...
    public function __construct() {
        ...
        if( 'on' == get_option( 'orddd_delivery_date_on_cart_page' ) ) {
            add_action( 'woocommerce_cart_collaterals', array( &$this, 'orddd_date_after_checkout_billing_form' ) );
            add_action( 'woocommerce_cart_collaterals', array( &$this, 'orddd_time_slot_after_checkout_billing_form' ) );  
            add_action( 'woocommerce_cart_collaterals', array( &$this, 'orddd_text_block_after_checkout_billing_form' ) );
        }
        ...
        add_action( 'woocommerce_checkout_update_order_meta', array( &$this, 'orddd_update_order_meta_delivery_date' ) );
        add_action( 'woocommerce_checkout_update_order_meta', array( &$this, 'orddd_update_order_meta_time_slot'  ) );
    ...
    public static function orddd_date_after_checkout_billing_form( $checkout = "" ) {    
        global $wpdb, $orddd_date_formats, $post, $woocommerce, $orddd_languages, $orddd_weekdays, $orddd_shipping_days, $current_user;
        $roles = array();
        if ( has_filter( 'orddd_disable_delivery_for_user_roles' ) ) {
            $roles = apply_filters( 'orddd_disable_delivery_for_user_roles', $roles );
        }
	...
	public static function orddd_time_slot_after_checkout_billing_form ( $checkout = "" ) {
        ...
        if( $is_delivery_enabled == 'yes' && ( orddd_process::woo_product_has_delivery() === 'on' ) && !in_array( $current_user_role, $roles ) ) {
		    ...
		    if ( get_option( 'orddd_enable_delivery_date' ) == 'on' && ( get_option( 'orddd_enable_time_slot' ) == 'on' || get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && 'yes' == $time_slots_for_shipping_methods ) ) {
                $orddd_is_delivery_calendar_enabled_for_custom = orddd_common::orddd_is_delivery_calendar_enabled_for_custom_delivery();
                if( 'delivery_calendar' == get_option( 'orddd_delivery_checkout_options' ) || 'yes' == $orddd_is_delivery_calendar_enabled_for_custom ) {
	...
	public static function orddd_validate_date() {
	    ...
	    if ( $date_mandatory == "Yes" ) {
            ...
            $delivery_date_label = orddd_common::orddd_get_delivery_date_field_label( $shipping_method, $product_category, $shipping_class, $location ); 

            if( $is_delivery_enabled == 'yes' && ( orddd_process::woo_product_has_delivery() === 'on' ) && ( 'yes' == orddd_common::orddd_is_delivery_enabled() ) && !in_array( $current_user_role, $roles ) && ( 'delivery_calendar' == get_option( 'orddd_delivery_checkout_options' ) || 'yes' == $orddd_is_delivery_calendar_enabled_for_custom ) ) {
	...
	public static function orddd_validate_time_slot() {
	    ...
	    if( $timeslot_mandatory == "Yes" )  {
            ...
            $delivery_time_label = orddd_common::orddd_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location ); 
     ...
     * @since 2.6
	 */
	public static function orddd_add_delivery_date_fee() {
		...
		if ( 'yes' != $add_delivery_charges_for_free_coupon_code ) {
    		...
    		foreach ( $applied_coupons as $applied_coupons_key => $applied_coupons_value ) {
                ...
                $orddd_free_shipping = ( version_compare( get_option( 'woocommerce_version' ), '3.0.0', ">="  ) ) ? $data[ 'free_shipping' ] : $applied_coupons_value->free_shipping;
                if ( 'yes' == $orddd_free_shipping ) {
		...
		if( $free_coupon_enabled != 'yes' ) {		
    		$delivery_date = $category_to_load = $shipping_class_to_load = $time_slot = $orddd_zone_id = '';
            $location = '';
    		...
    		if ( isset( $_POST[ 'post_data' ] ) ) {
    			...
    			$time_slot_type = preg_match( '/&time_slot=(.*?)&/', $_POST[ 'post_data' ], $matches );
    			if ( isset( $matches[ 1 ] ) ) {
            ...
            if( is_array( $categories ) && count( $categories ) == 0 ) {
                $categories = orddd_common::orddd_get_cart_product_categories( false, '' );
            }
            ...
            if( get_option( 'orddd_enable_shipping_based_delivery' ) ) {
                $shipping_settings_to_check = orddd_common::orddd_get_custom_settings( $shipping_method, $shipping_classes, $categories, $location );
    		...
    		if( $time_slot != '' && $time_slot != 'choose' && $time_slot != 'NA' && $time_slot != 'select' ) {
                ...
                if( $time_slot_fees_to_add > 0 && $time_slot_fees_to_add != "" ) {
                    if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
                        ...
                        $woocommerce->cart->add_fee( __( $time_slot_charges_label, 'order-delivery-date' ), $time_slot_fees_to_add , true, $shipping_tax_class );
                    } else {
                        $woocommerce->cart->add_fee( __( $time_slot_charges_label, 'order-delivery-date' ), $time_slot_fees_to_add , false );
                    }
    		...
    		if ( $delivery_date != '' ) {
                ...
                if ( $fees_to_add > 0 ) {
                    if( isset( $delivery_charges_label ) && $delivery_charges_label != '' ) {
                        if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
                            $shipping_tax_class = get_option( 'woocommerce_shipping_tax_class' );
                            $woocommerce->cart->add_fee( __( $delivery_charges_label, 'order-delivery-date' ), $fees_to_add , true, $shipping_tax_class );
                        } else {
                            $woocommerce->cart->add_fee( __( $delivery_charges_label, 'order-delivery-date' ), $fees_to_add , false );
                        }
                    } else {
                        if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
                            $shipping_tax_class = get_option( 'woocommerce_shipping_tax_class' );
                            $woocommerce->cart->add_fee( __( 'Delivery Charges', 'order-delivery-date' ), $fees_to_add , true, $shipping_tax_class );
                        } else {
                            $woocommerce->cart->add_fee( __( 'Delivery Charges', 'order-delivery-date' ), $fees_to_add , false );
                        }
                    }
                ...
                if ( $same_day_fees > 0 ) {
                    ...
                    if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
                        $shipping_tax_class = get_option( 'woocommerce_shipping_tax_class' );
                        $woocommerce->cart->add_fee( __( $same_day_delivery_charges_label, 'order-delivery-date' ), $same_day_fees, true, $shipping_tax_class );
                    } else {
                        $woocommerce->cart->add_fee( __( $same_day_delivery_charges_label, 'order-delivery-date' ), $same_day_fees, false );
                    }
                ...
                if ( $next_day_fees > 0 ) {
                    ...
                    if( get_option( 'orddd_enable_tax_calculation_for_delivery_charges' ) == 'on' ) {
                        $shipping_tax_class = get_option( 'woocommerce_shipping_tax_class' );
                        $woocommerce->cart->add_fee( __( $next_day_delivery_charges_label, 'order-delivery-date' ), $next_day_fees, true, $shipping_tax_class );
                    } else {
                        $woocommerce->cart->add_fee( __( $next_day_delivery_charges_label, 'order-delivery-date' ), $next_day_fees, false );
                    }
	...
	public static function check_for_time_slot_orddd() {
        ...
        $session_time_slot = '';
        if( isset( $_POST[ 'time_slot_session' ] ) && 'on' == get_option( 'orddd_delivery_date_on_cart_page' ) ) {
        ...
        foreach ( $time_slots_to_show_timestamp as $key => $value ) {
            ...
            $sel = '';
            if( $session_time_slot != '' && $key == $session_time_slot ) {
                $sel = 'selected';
            } else if( ( $time_slot_for_order != '' && $key == $time_slot_for_order ) || ( $time_slot_for_multiple != '' && $key == $time_slot_for_multiple ) ) {
                $sel = 'selected';
            } else if ( $i == 1 && get_option( 'orddd_auto_populate_first_available_time_slot' ) == 'on' ) {
                $sel = 'selected';
            }

            $timeslot_charges = orddd_common::orddd_get_timeslot_charges( $key );
	...
	public static function orddd_update_order_meta_delivery_date( $order_id ) {
		...
		if ( isset( $_POST[ 'e_deliverydate' ] ) && $_POST[ 'e_deliverydate' ] != '' ) {
			...
			do_action( 'orddd_after_delivery_date_update', $order_id, $delivery_date );
		} else {
		    $is_delivery_enabled = orddd_common::orddd_is_delivery_enabled();
        ...
        if( isset( $_SESSION[ 'e_deliverydate' ] ) ) {
            unset( $_SESSION[ 'e_deliverydate' ] );
        }

        if( isset( $_SESSION[ 'h_deliverydate' ] ) ) {
            unset( $_SESSION[ 'h_deliverydate' ] );
        }

        if( isset( $_SESSION[ 'time_slot' ] ) ) {
	...
	public static function orddd_update_order_meta_time_slot( $order_id ) {
        ...
        $time_slot_label = orddd_common::orddd_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location ); 
	...
	public static function orddd_add_delivery_date_to_order_page_woo( $order ) {
        ...
        foreach ( $order_statuses as $order_status_key => $order_status_name ) {
            $order_statuses_arr[] = $order_status_key;
        }       
        ...
        if( $delivery_date_formatted != '' && ( !is_account_page() || 'no' == $orddd_hide_delivery_on_my_account ) ) {
        	...
        	if( 'on' == get_option( 'orddd_allow_customers_to_edit_date' ) &&  is_account_page() ) {
                ...
                if( in_array( $order_post_status, $order_statuses_arr ) ) {
        		    echo '<span class="orddd-edit-div">
        		        <a href="javascript:void(0)" id="edit_delivery_date">Edit</a>
                    </span>';
                }
        	}
	...
	public static function orddd_my_account_my_orders_query( $order ) {
	    global $orddd_date_formats, $post;
	    ...
	    if( $delivery_date_formatted != '' ) {
			echo '<p>' . $delivery_date_formatted . '</p>';
		}

		$order_page_time_slot = orddd_common::orddd_get_order_timeslot( $order_id );
		if( $order_page_time_slot != "" && $order_page_time_slot != '' ) {
		    echo '<p>' . $order_page_time_slot . '</p>';
		}
	...
	public static function orddd_update_delivery_date() {
        $order_id = $delivery_date = $time_slot = '';
	    if( isset( $_POST[ 'order_id' ] ) ) {
	        $order_id = $_POST[ 'order_id' ];
	    }
     ...
     * @since 1.0
	 */
	public static function orddd_update_lockout_days( $delivery_date ) {
	    global $wpdb;
	    if( get_option( 'orddd_lockout_date_quantity_based' ) == 'on' ) {
    ...
    public static function orddd_availability_check() {
        ...
        if( ( orddd_process::woo_product_has_delivery() === 'on' ) && ( 'yes' == orddd_common::orddd_is_delivery_enabled() ) && ( !in_array( $current_user_role, $roles ) ) ) {
	    	...
	    	if( isset( $_POST[ 'time_slot' ] ) && $_POST[ 'time_slot' ] != '' && $_POST[ 'time_slot' ] != 'choose' && $_POST[ 'time_slot' ] != 'NA' && $_POST[ 'time_slot' ] != 'select' ) {
	        	$time_slot = $_POST[ 'time_slot' ];
	        } 
	        ...
	        if( '' != $delivery_date ) {
	                	...
	                	foreach ( $results as $key => $value ) {
	                        ...
	                        if( in_array( $shipping_method, $shipping_methods ) ) {
	                            ...
	                            if( isset( $shipping_settings[ 'date_lockout' ] ) && $shipping_settings[ 'date_lockout' ] != '' && $shipping_settings[ 'date_lockout' ] != '0') {
	                                if( isset( $_POST[ 'h_deliverydate' ] ) && $_POST[ 'h_deliverydate' ] != '' ) {
	                                    $lockout_date_format = date( ORDDD_LOCKOUT_DATE_FORMAT, strtotime( $_POST[ 'h_deliverydate' ] ) );
	                                } 
	    	                            ...
	    	                            if( $tv != '' ) {
	                                        ...
	                                        if( $timeslot_values[ 'time_slot' ] == $select_time_slot_str ) {
	    	                                    ...
	    	                                    if( $timeslot_values[ 'lockout' ] != 0 && $timeslot_values[ 'lockout' ] != "" ) {
	    	                                        ...
	    	                                        if( $available_time_slot_quantities == "" ) {
	    	                                            $available_time_slot_quantities = $lockout;
	    	                                        }
	                                                ...
	                                                if( $available_time_slot_quantities < $total_quantities && $available_time_slot_quantities != 0 && $available_time_slot_quantities != '' && $is_custom_date_lockout_reached == 'No' ) {
	    	                                            $message = $lockout_date . " has only " . $available_time_slot_quantities . " deliveries available for the time slot " . $_POST[ 'time_slot' ] . ".";
	    	                                            wc_add_notice( __( $message, 'order-delivery-date' ), $notice_type = 'error');
	    	                                        }
	        	...
	        	if( $shipping_based_date == "No" ) {
	                ...
	                if( get_option( 'orddd_lockout_date_after_orders' ) != "0" && get_option( 'orddd_lockout_date_after_orders' ) != "" ) {
	                    $lockout_days = get_option( 'orddd_lockout_days' );
	                    if ( $lockout_days == '' || $lockout_days == '{}' || $lockout_days == '[]' || $lockout_days == 'null' ) {
	                        $lockout_days_arr = array();
	                    } else {
	                    ...
	                    if( is_array( $lockout_days_arr ) && count( $lockout_days_arr ) > 0 ) {
	                        foreach ( $lockout_days_arr as $k => $v ) {
	                            if ( $lockout_date_format == $v->d ) {
	                                $available_date_quantities = get_option( 'orddd_lockout_date_after_orders' ) - $v->o;
	                                break;
	                            } 
	                        }
	                    } else {
	                        ...
	                        if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 && get_option( 'orddd_enable_specific_delivery_dates' ) == "on" ) {
	                            ...
	                            foreach( $dd as $dkey => $dval ) {
	                                if( in_array( $lockout_date_format, $specific_dates_arr ) ) {
	                                    ...
	                                    $weekday = date( 'w', strtotime( $_POST[ 'h_deliverydate' ] ) );
		    ...
		    if( get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
                ...
                if( "Yes" == $shipping_settings_exists ) {
                    ...
                    if( is_array( $same_day_cut_off ) && count( $same_day_cut_off ) > 0 ) {
                        ...
                        if( isset( $shipping_settings_exists[ 'same_day' ] ) ) {
                            $custom_same_day = $shipping_settings_exists[ 'same_day' ];       
                        }
                    }
                    ...
                    if( is_array( $next_day_cut_off ) && count( $next_day_cut_off ) > 0 ) {
                        ...
                        if( isset( $shipping_settings_exists[ 'next_day' ] ) ) {
                            $custom_next_day = $shipping_settings_exists[ 'next_day' ];       
                        }
                    }
