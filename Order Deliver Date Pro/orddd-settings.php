...
include_once( 'includes/orddd-date-settings.php' );
include_once( 'includes/orddd-shipping-days-settings.php' );
include_once( 'includes/orddd-time-settings.php' );
include_once( 'includes/orddd-holidays-settings.php' );
include_once( 'includes/orddd-appearance-settings.php' );
include_once( 'includes/orddd-delivery-days-settings.php' );
include_once( 'includes/orddd-time-slot-settings.php' );
include_once( 'includes/orddd-additional-settings.php' );
include_once( 'includes/orddd-shipping-based-settings.php' );
include_once( 'includes/orddd-calendar-sync-settings.php' );
...
class orddd_settings {
    ...
    */ 
    ...
    public function order_delivery_date_admin_settings() {
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_date_settings_section',
            array ( '&nbsp' . __( 'Select weekdays for deliveries.', 'order-delivery-date' ) )
        );
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_date_settings_section',
            array ( '&nbsp' . __( 'Enable this setting to add Additional charges, Additional charges\' checkout label, Same day cut-off time, Next day cut-off time and Minimum Delivery Time (in hours) for each weekday.' ) )
        );
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_shipping_days_settings_section',
            array ('&nbsp' . __( 'Select weekdays for shipping.', 'order-delivery-date' ) )
        );
    ...
    public  function orddd_delivery_days_settings() {
    	...
    	add_settings_field(
    	   ...
    	   'orddd_delivery_days_page',
    	   'orddd_delivery_days_section',
    	   array( __( '<br>Add delivery charges (if applicable) and select the specific delivery date.', 'order-delivery-date' ) )
    	);
    ...
    public  function orddd_time_slot_settings() {
        ...
        add_settings_field(
            ...
            'orddd_time_slot_page',
            'orddd_add_time_slot_section',
            array( '<br>' . __( 'Start time for the time slot.', 'order-delivery-date' ) )
        );
        ...
        add_settings_field(
            ...
            'orddd_time_slot_page',
            'orddd_add_time_slot_section',
            array( '<br>' . __( 'End time for the time slot.', 'order-delivery-date' ) )
        );
    ...
    public function orddd_calendar_sync_settings_callback() {
        ...
        add_settings_field(
            ...
            'orddd_calendar_sync_settings_page',
            'orddd_calendar_sync_general_settings_section',
            array ( __( '<br>Enter the text that will be used as location field in event of the Calendar. If left empty, website description is sent instead. <br><i>Note: You can use ADDRESS, FULL_ADDRESS and CITY placeholders which will be replaced by their real values.</i>', 'order-delivery-date' ) )
        );
        ...
        add_settings_field(
            ...
            __( 'Show Add to Calendar button on Delivery Calendar page', 'order-delivery-date' ),
            array( 'orddd_calendar_sync_settings', 'orddd_admin_add_to_calendar_delivery_calendar_callback' ),
            'orddd_calendar_sync_settings_page',
            'orddd_calendar_sync_admin_settings_section',
            array( __( 'Show "Add to Calendar" button on the Order Delivery Date -> Delivery Calendar page.<br><i>Note: This button can be used to export the already placed orders with future deliveries from the current date to the calendar used above.</i>', 'order-delivery-date' ) )
        );
	...
	public function order_delivery_date_settings() {
		...
		if( $action == 'general_settings' ) {
		    ...
		    if ( $section == 'time_slot' ) {
                ...
                ?>
                <a href="admin.php?page=order_delivery_date&action=general_settings&section=block_time_slot_settings" class="block_time_slot"><?php _e( 'Block Time Slots', 'order-delivery-date' );?> </a>
                <?php 
            ...
            if( is_array( $results ) && count( $results ) == 0 && get_option( 'orddd_enable_shipping_based_delivery' ) == 'on' ) {
	...
	public function orddd_delete_settings() {
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'shipping_based' ) ) {
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'delivery_dates' )  ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
    	        $delivery_date = array();
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && ( isset( $_GET[ 'tab' ] ) && sanitize_text_field( $_GET[ 'tab' ] ) == 'general_settings' ) && ( isset( $_GET[ 'section' ] ) && sanitize_text_field( $_GET[ 'section' ] ) == 'holidays' ) ) {
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || ( isset( $_GET[ 'action2' ] ) && sanitize_text_field( $_GET[ 'action2' ] ) == 'orddd_delete') ) {
                $holiday = array();
    ...
    public function orddd_edit_delivery_field_for_product_category( $term, $taxonomy ) {	
        $display_type = get_woocommerce_term_meta( $term->term_id, 'orddd_delivery_date_for_product_category', true );
	...
	public function orddd_save_category_fields( $term_id, $tt_id, $taxonomy ) {	
        if ( isset( $_POST[ 'orddd_delivery_date_for_product_category' ] ) ) {
            update_woocommerce_term_meta( $term_id, 'orddd_delivery_date_for_product_category', sanitize_text_field( esc_attr( $_POST[ 'orddd_delivery_date_for_product_category' ] ) ) );
        } else {
            if( ( isset( $_POST[ 'newproduct_cat' ] ) && $_POST[ 'newproduct_cat' ] != '' ) || ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'wpml_save_job_ajax' ) ) {
                update_woocommerce_term_meta( $term_id, 'orddd_delivery_date_for_product_category', 'on' );
            } else {
                update_woocommerce_term_meta( $term_id, 'orddd_delivery_date_for_product_category', '' );
            }
        }
	...
	*/
	...
	public function orddd_product_cat_column( $columns, $column, $id ) {			
        if ( $column == 'delivery' ) {
            $delivery_date  = get_woocommerce_term_meta( $id, 'orddd_delivery_date_for_product_category', true );
